function addAvgToTable(players) {

    let tbody = document.getElementById('avgLeaderboardtbody');
    let trArray = [];
    let pos = 0;
...
getLeaderboardAvg(addAvgToTable, 1);


function addBestToTable(players) {

    let tbody = document.getElementById('bestLeaderboardtbody');
    let trArray = [];
    let pos = 0;
    ...
    players.forEach(function (player) {
        ...
        tdArray[2].appendChild(document.createTextNode(player.last_name));
        tdArray[3].appendChild(document.createTextNode(player.name));
        tdArray[4].appendChild(document.createTextNode(player.date));
        tdArray[5].appendChild(document.createTextNode(player.par));
        tdArray[6].appendChild(document.createTextNode(player.throws));
        ...
        let sum = Math.round((player.throws - player.par) * 10) / 10;
        if (sum >= 0) {
...
getLeaderboardBest(addBestToTable, 1);